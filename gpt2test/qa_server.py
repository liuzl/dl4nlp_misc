...
unk_idx = open("vocab.txt").read().split('\n').index('[UNK]')
device = "cuda" if torch.cuda.is_available() else "cpu"
tokenizer = tokenization_bert.BertTokenizer(vocab_file="vocab.txt")
model = GPT2LMHeadModel.from_pretrained("sifa")
model.to(device)
model.eval()
...
@app.route('/')
def index():
    return app.send_static_file('sifa.html')
